...
BOT_TOKEN = '8713237079:AAFzaZGqkHFEN3q7onaJtxjSJKv3BPRjaHA'
...
async def load_sites(user_id=None):
    ...
    # Master pool for all users
    global_sites = await get_file_lines(SITES_FILE)
    if global_sites:
        return global_sites
    for admin in ADMIN_ID:
        adm_sites = await db_manager.get_all_user_sites(admin)
        if adm_sites:
            return [item['url'] for item in adm_sites]
    return []
...
async def load_sites_with_price(user_id=None, min_price=0.0, max_price=30.0):
    ...
    if global_sites:
        matching = [s for s in global_sites if min_price <= s.get('price', 0.0) <= max_price]
        if matching:
            return matching
    for admin in ADMIN_ID:
        adm_sites = await db_manager.get_user_sites(admin, min_price, max_price)
        if adm_sites:
            return adm_sites
    file_sites = await get_file_lines(SITES_FILE)
    if file_sites:
        return [{'url': s, 'price': 0.0} for s in file_sites]
    return []
...
async def load_proxies(user_id=None):
    ...
    for admin in ADMIN_ID:
        adm_proxies = await db_manager.get_user_proxies(admin)
        if adm_proxies:
            return adm_proxies
    return []
...
async def check_card_with_retry(card, sites, proxies, max_retries=2, session=None, max_price=None):
    if not sites:
...
async def start_mass_check(user_id, cards, sites, event, max_price=None):
    ...
    if not proxies:
        if user_id in active_user_checks:
            active_user_checks.remove(user_id)
        msg_text = "❌ <b>Nᴏ ᴘʀᴏxɪᴇs ᴀᴠᴀɪʟᴀʙʟᴇ!</b>\n\nPʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ ᴀɴ ᴀᴅᴍɪɴ ᴛᴏ ʟᴏᴀᴅ ᴘʀᴏxɪᴇs."
        try:
    ...
    try:
        ...
        # High-performance concurrency tuned for 6 Railway APIs x 5 replicas (30 replicas, 240 uvicorn workers)
        # 120 concurrent workers gives ~20 req/API = ~4 req/replica, perfectly saturating the cluster
        max_threads = min(len(proxy_list) * 2, 120) if proxy_list else 60
        ...
        async def worker(shared_session):
            while not queue.empty() and session_key in active_sessions:
                ...
                if not sites or not proxy_list:
                    break
                async with api_sem:
                    res = await check_card_with_retry(card, sites, proxy_list, max_retries=2, session=shared_session, max_price=max_price)
        ...
        timeout = aiohttp.ClientTimeout(total=35, connect=8, sock_read=30)
        connector = aiohttp.TCPConnector(limit=1000, limit_per_host=100, enable_cleanup_closed=True, ttl_dns_cache=600, use_dns_cache=True, keepalive_timeout=75)
...
🛒 <b>1. Cᴀʀᴅ Cʜᴇᴄᴋɪɴɢ (Pʟᴀɴ Rᴇǫᴜɪʀᴇᴅ)</b>
...
🛠️ <b>2. Fʀᴇᴇ Uᴛɪʟɪᴛɪᴇs (Nᴏ Pʟᴀɴ Nᴇᴇᴅᴇᴅ)</b>
...
async def price_filter_callback(event):
    ...
    sites_data = await load_sites_with_price(user_id)
    if not sites_data:
        await safe_edit(event, premium_emoji("❌ <b>Nᴏ sɪᴛᴇs ᴀᴠᴀɪʟᴀʙʟᴇ!</b>\n\nPʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ ᴀɴ ᴀᴅᴍɪɴ ᴛᴏ ʟᴏᴀᴅ sɪᴛᴇs."), parse_mode='html')
        return
...
async def single_cc_check(event):
    ...
    if not sites:
        ...
        return
    if not proxies:
        await event.reply(premium_emoji("❌ Nᴏ ᴘʀᴏxɪᴇs ᴀᴠᴀɪʟᴀʙʟᴇ! Pʟᴇᴀsᴇ ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ."), parse_mode='html')
        return
...
async def add_proxy_command(event):
    ...
    if user_id not in ADMIN_ID:
        await event.reply(
            premium_emoji("⛔ <b>Aᴄᴄᴇss Dᴇɴɪᴇᴅ!</b>\n\nPʀᴏxɪᴇs ᴀʀᴇ ᴍᴀɴᴀɢᴇᴅ ᴄᴇɴᴛʀᴀʟʟʏ ʙʏ ᴀᴅᴍɪɴs. Yᴏᴜ ᴄᴀɴ ᴅɪʀᴇᴄᴛʟʏ ᴜsᴇ <code>/chk</code> ᴏʀ <code>/cc</code> ᴛᴏ ᴄʜᴇᴄᴋ ᴄᴀʀᴅs!"),
            parse_mode='html'
        )
        return
    try:
        ...
        to_test = list(dict.fromkeys(proxies_to_add))
        ...
        try:
            current_admin_proxies = await db_manager.get_user_proxies(user_id)
            async with aiofiles.open(PROXY_FILE, 'w', encoding='utf-8') as f:
                for p_item in current_admin_proxies:
                    await f.write(f"{p_item}\n")
        except Exception as e:
            print(f"Error syncing master proxies: {e}")
...
async def check_proxies_command(event):
    user_id = event.sender_id
    if user_id not in ADMIN_ID:
        await event.reply(premium_emoji("⛔ <b>Aᴄᴄᴇss Dᴇɴɪᴇᴅ!</b>\n\nPʀᴏxʏ ᴍᴀɴᴀɢᴇᴍᴇɴᴛ ɪs ʀᴇsᴛʀɪᴄᴛᴇᴅ ᴛᴏ ᴀᴅᴍɪɴs."), parse_mode='html')
        return
...
async def add_sites_command(event):
    ...
    if user_id not in ADMIN_ID:
        await event.reply(
            premium_emoji("⛔ <b>Aᴄᴄᴇss Dᴇɴɪᴇᴅ!</b>\n\nSɪᴛᴇs ᴀʀᴇ ᴍᴀɴᴀɢᴇᴅ ᴄᴇɴᴛʀᴀʟʟʏ ʙʏ ᴀᴅᴍɪɴs. Yᴏᴜ ᴄᴀɴ ᴅɪʀᴇᴄᴛʟʏ ᴜsᴇ <code>/chk</code> ᴏʀ <code>/cc</code> ᴛᴏ ᴄʜᴇᴄᴋ ᴄᴀʀᴅs!"),
            parse_mode='html'
        )
        return
    try:
        ...
        if not proxies:
            await event.reply(
                premium_emoji("❌ <b>Nᴏ ᴘʀᴏxɪᴇs ғᴏᴜɴᴅ!</b>\n\nPʟᴇᴀsᴇ ᴀᴅᴅ ᴘʀᴏxɪᴇs ᴜsɪɴɢ <code>/addproxy</code> ʙᴇғᴏʀᴇ ᴀᴅᴅɪɴɢ sɪᴛᴇs."),
                parse_mode='html'
            )
            return
...
async def site_command(event):
    user_id = event.sender_id
    if user_id not in ADMIN_ID:
        await event.reply(premium_emoji("⛔ <b>Aᴄᴄᴇss Dᴇɴɪᴇᴅ!</b>\n\nSɪᴛᴇ ᴍᴀɴᴀɢᴇᴍᴇɴᴛ ɪs ʀᴇsᴛʀɪᴄᴛᴇᴅ ᴛᴏ ᴀᴅᴍɪɴs."), parse_mode='html')
        return
...
async def get_sites_command(event):
    user_id = event.sender_id
    if user_id not in ADMIN_ID:
        await event.reply(premium_emoji("⛔ <b>Aᴄᴄᴇss Dᴇɴɪᴇᴅ!</b>\n\nSɪᴛᴇ ᴇxᴘᴏʀᴛ ɪs ʀᴇsᴛʀɪᴄᴛᴇᴅ ᴛᴏ ᴀᴅᴍɪɴs."), parse_mode='html')
        return
    sites_with_p = await db_manager.get_all_user_sites(user_id)
    if not sites_with_p and user_id in ADMIN_ID:
        global_s = await get_file_lines(SITES_FILE)
...
async def get_proxy_command(event):
    user_id = event.sender_id
    if user_id not in ADMIN_ID:
        await event.reply(premium_emoji("⛔ <b>Aᴄᴄᴇss Dᴇɴɪᴇᴅ!</b>\n\nPʀᴏxʏ ᴇxᴘᴏʀᴛ ɪs ʀᴇsᴛʀɪᴄᴛᴇᴅ ᴛᴏ ᴀᴅᴍɪɴs."), parse_mode='html')
        return
    proxies = await db_manager.get_user_proxies(user_id)
    if not proxies and user_id in ADMIN_ID:
        proxies = await get_file_lines(PROXY_FILE)
...
🌐 <b>Pʀɪᴠᴀᴛᴇ Pᴏᴏʟ:</b>
┣ 📂 Sɪᴛᴇs Sᴀᴠᴇᴅ: <code>{u['sites_count']}</code>
┗ ⚡ Pʀᴏxɪᴇs Sᴀᴠᴇᴅ: <code>{u['proxies_count']}</code>
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async def probe(api):
                    try:
                        ...
                        pass
                    return None
                results = await asyncio.gather(*[probe(api) for api in API_SERVERS])
